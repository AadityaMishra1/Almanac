...
function isRowPastDue(date: string): boolean {
  try {
    return startOfDay(parseISO(date)) < startOfDay(new Date());
  } catch {
    return false;
  }
}
...
export function EventsTable({
  ...
  onDeleteRow?: (idx: number, id?: string) => void;
}) {
            ...
            <div className="flex flex-col sm:flex-row items-start gap-4">
              ...
              <div className="flex-1 min-w-0 space-y-2">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {isRowPastDue(row.date) && (
                    <span className="inline-flex items-center gap-1 text-[11px] font-medium uppercase tracking-wider rounded-md px-2 py-0.5 bg-red-50 text-red-600 dark:bg-red-950/30 dark:text-red-400">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                {onDeleteRow && (
                  <button
                    onClick={() =>
                      onDeleteRow(idx, (row as Row & { id?: string }).id)
                    }
