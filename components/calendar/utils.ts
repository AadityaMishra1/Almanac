...
const COURSE_COLORS: ColorScheme[] = [
  ...
  },
  {
    ...
    gradient:
      "from-violet-50 to-violet-100 dark:from-violet-950/30 dark:to-violet-900/30",
  ...
  },
  {
    ...
    accent: "bg-cyan-500",
    gradient:
      "from-cyan-50 to-cyan-100 dark:from-cyan-950/30 dark:to-cyan-900/30",
  ...
  },
  {
    ...
    gradient:
      "from-fuchsia-50 to-fuchsia-100 dark:from-fuchsia-950/30 dark:to-fuchsia-900/30",
...
// ── Past-Due Colors ──────────────────────────────────────────────────
const PAST_DUE_COLORS: ColorScheme = {
  bg: "bg-red-50/80 dark:bg-red-950/30",
  border: "border-red-400 dark:border-red-700",
  text: "text-red-700 dark:text-red-300",
  accent: "bg-red-500",
  gradient: "from-red-50 to-red-100 dark:from-red-950/30 dark:to-red-900/30",
  hover: "hover:bg-red-100/80 dark:hover:bg-red-950/50",
};

/**
 * Check if an assignment-type event is past due (start_time before today).
 */
export const isEventPastDue = (event: UnifiedEvent): boolean => {
  if (event.type !== "assignment") return false;
  return startOfDay(parseISO(event.start_time)) < startOfDay(new Date());
};
 ...
 * Priority: past-due (red) > course-based color > type-based fallback.
 */
export const getEventColor = (event: UnifiedEvent): ColorScheme => {
  // Past-due assignments get red override
  if (isEventPastDue(event)) return PAST_DUE_COLORS;
