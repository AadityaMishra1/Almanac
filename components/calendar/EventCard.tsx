...
export function EventCard({ event, view, onClick, className }: EventCardProps) {
  ...
  const isHighlighted = useChatStore((s) =>
    s.highlightedEventIds.has(event.id),
  );
  ...
  const pastDue = isEventPastDue(event);
  ...
  return (
    <div
      ...
      aria-label={`${event.title} - ${event.assignment_type || "event"}${pastDue ? " - Past due" : ""}`}
    >
      ...
      {!isCompact && (
        <div className="flex items-center gap-2 mt-0.5">
          ...
          {pastDue && (
            <span className="text-[10px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded bg-red-100 text-red-600 dark:bg-red-950/40 dark:text-red-400">
              Past due
            </span>
          )}
        </div>
